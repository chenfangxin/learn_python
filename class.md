...
> `dir()`函数返回对象的所有属性的列表
> 实例的`__dict__`属性是一个字典，仅存放该实例拥有的属性，包括从父类继承来的属性
> 类的`__dict__`存放该类拥有的属性，并不包含父类的属性

#### 使用`__slots__`

Python中的新式类，可以通过名为`__slots__`的类成员，来控制对象实例可用的属性名称，例如：
...
p.name='cfx'
p.age=30
p.length=120   #出现AttributeError
```

**注意：** `__slots__`属性只对新式类有效。而且如果子类也定义了自己`__slots__`属性，则子类对象可用的属性名称为子类与父类`__slots__`的合集；如果子类没有定义`__slots__`属性，则子类对象不受父类的`__slots__`约束。使用了`__slots__`属性，就不能进行多重继承

`__slots__`是优化Python性能的一种方法。使用了`__slots__`的类，就不自动生成`__dict__`和`__weakref__`了，可以减少空间占用，并提高属性查找效率。
...
由于Python是`动态类型`语言，其函数参数类型的确定，不是按照传入参数的类型本身或其继承关系，而是这个参数所提供的接口。

> 鸭子类型：当一只鸟，走起来像鸭子，游泳像鸭子，叫起来也像鸭子，那这只鸟就是鸭子
...
def test(arg):
		arg.prt()  # 只要提供`prt`接口，任何对象都可以作为test的参数
